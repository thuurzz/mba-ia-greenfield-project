...
export const envValidationSchema = Joi.object({
  ...
  STORAGE_ENDPOINT: Joi.string().default('minio:9000'),
  STORAGE_REGION: Joi.string().default('us-east-1'),
  STORAGE_ACCESS_KEY: Joi.string().required(),
  STORAGE_SECRET_KEY: Joi.string().required(),
  STORAGE_BUCKET: Joi.string().default('streamtube-videos'),
  STORAGE_PUBLIC_BUCKET: Joi.string().default('streamtube-public'),
  STORAGE_USE_SSL: Joi.string().valid('true', 'false').default('false'),
  REDIS_HOST: Joi.string().default('redis'),
  REDIS_PORT: Joi.number().default(6379),
  QUEUE_VIDEO_PROCESSING: Joi.string().default('video-processing'),
});
