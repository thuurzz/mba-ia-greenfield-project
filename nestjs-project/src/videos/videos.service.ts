...
export class VideosService {
  ...
  async findByChannelPublic(channelId: string, opts: { cursor?: string; limit: number }) {
    ...
    if (opts.cursor) {
      const cursorVideo = await this.videoRepository.findOne({ where: { id: opts.cursor } });
      if (cursorVideo?.publishedAt) {
        qb.andWhere('v.published_at < :cursorDate', { cursorDate: cursorVideo.publishedAt });
      }
    }
  ...
  async findSuggested(videoId: string, categoryId: number | null, limit = 12) {
    const baseWhere: any = { status: VideoStatus.READY, publishedAt: Not(IsNull()), visibility: 'public' };
    if (!categoryId) {
